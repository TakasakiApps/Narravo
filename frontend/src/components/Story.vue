<template>
    <div style="overflow-y:hidden; display: inline-block; ">
        <div v-masonry transition-duration="0.3s" item-selector=".item-card" fit-width="true" gutter="20" class="body">
            <div v-masonry-tile class="item-card" @click="openbook(props.item)">
                <el-card :body-style="{ padding: '0px', }" v-contextmenu:contextmenu shadow="hover">
                    <img :src="bkcover" class="image" />
                    <div style="padding: 5px">
                        <div class="title">
                            <h1>{{ props.item.name }}</h1>
                            <el-icon size="25">
                                <svg t="1698674595063" class="icon" viewBox="0 0 1024 1024" version="1.1"
                                    xmlns="http://www.w3.org/2000/svg" p-id="4957" width="32" height="32">
                                    <path
                                        ...
                                        fill="currentColor" p-id="4958"></path>
                                </svg>
                            </el-icon>
                        </div>
                        ...
                        <div class="bottom">
                            ...
                            <p text class="author">{{ props.item.author.name }}</p>
                        </div>
                    </div>
                </el-card>
            </div>
    ...
    <v-contextmenu ref="contextmenu">
        ...
        <v-contextmenu-item><span @click="addbookliem(props.item)">加入书架</span></v-contextmenu-item>
    </v-contextmenu>
</template>
...
const bkcover = ref(null)
function getCover() {
    client.get('/api/v1/assets/fetch/image', {
        params: {
            token: localStorage.token,
            type: 'cover',
            id: props.item.coverId
        },
    }).then(
        res => {
            bkcover.value = res.request.responseURL
            // console.log(bkcover.value)
        }
    ).catch(
        err => {
            console.log(err)
...
function addbookliem(e) {
    ...
    ipcRenderer.send('addfrbookliem', bookdata)
}
ipcRenderer.on('storyOkadd', (e, data) => {
    setTimeout(() => {
